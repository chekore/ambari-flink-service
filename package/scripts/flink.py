...
class Master(Script):
    def install(self, env):
        ...
        if params.setup_prebuilt:
            ...
            if not os.path.exists(params.temp_file):
                Execute("wget %s -O %s -a %s" % (params.flink_download_url, params.temp_file, params.flink_log_file),
                        user=params.flink_user)
            Execute("tar -zxvf %s -C %s >> %s" % (params.temp_file, params.flink_install_dir, params.flink_log_file),
                    user=params.flink_user)
            Execute('mv %s/*/* %s/' % (params.flink_install_dir, params.flink_install_dir), user=params.flink_user)
    ...
    def start(self, env):
        ...
        Execute(
            "yarn application -list 2>/dev/null | awk '/" + params.flink_appname + "/ {print $1}' | head -n1 > " + status_params.flink_pid_file,
            user=params.flink_user)
